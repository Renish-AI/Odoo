...
const PIE_COLORS = ['#38bdf8', '#818cf8', '#f59e0b', '#10b981', '#06b6d4', '#ec4899', '#94a3b8'];
...
export const FinancialCockpit = ({ trip }) => {
  ...
  const avgCostPerDay = summary.avgCostPerDay;
  const costPerTraveler = Math.round(totalEstimatedCost / Math.max(1, travelersCount));
  const percentSpent = summary.percentageUsed;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-slate-900/80 border border-ocean-500/20 shadow-2xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <Receipt className="w-5 h-5 text-cyan-400" />
              <h3 className="text-lg font-black text-white tracking-tight">Trip Financial Cockpit</h3>
            </div>
        ...
        >
          {summary.alertLevel === 'healthy' && <CheckCircle2 className="w-5 h-5 shrink-0" />}
          {summary.alertLevel === 'warning' && <AlertTriangle className="w-5 h-5 shrink-0" />}
          {summary.alertLevel === 'over' && <AlertCircle className="w-5 h-5 shrink-0" />}
          <div className="text-xs font-bold">{summary.alertMessage}</div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800">
            ...
            <div className="text-[10px] text-slate-500 mt-0.5">Ceiling Target</div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800">
            ...
            <div className={`text-xl font-black mt-1 ${remainingBudget >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800">
            ...
            <div className="text-[10px] text-slate-500 mt-0.5">{summary.tripDays} Days Total</div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-ocean-500/20 shadow-xl space-y-4">
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-ocean-500/20 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-sm font-bold text-white">Category Breakdown Comparison</h4>
            </div>
      ...
      <div className="p-6 rounded-3xl bg-slate-900/60 border border-ocean-500/20 shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-ocean-500 to-cyan-400 hover:from-ocean-400 hover:to-cyan-300 text-slate-950 font-black text-xs shadow-lg shadow-cyan-500/20 transition-all hover:scale-105"
          >
        ...
        {filteredExpenses.length === 0 ? (
          <div className="text-center py-10 border border-dashed border-slate-800 rounded-2xl text-xs text-slate-500">
            No logged expenses in this category. (Activities & Transit costs are automatically calculated above).
          </div>
        ) : (
                ...
                {filteredExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-2.5 px-3">
                      <span className="px-2 py-0.5 rounded-md bg-slate-800 text-cyan-300 text-[10px] font-bold">
                        {exp.category}
                      </span>
                    </td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-slate-900 border border-ocean-500/30 rounded-3xl p-6 shadow-2xl">
            <h4 className="text-base font-extrabold text-white mb-4">Log Travel Expense</h4>
            ...
            <form onSubmit={handleAddExpense} className="space-y-3.5">
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-800 mt-4">
                ...
                <button
                  type="submit"
                  disabled={!amount}
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-ocean-500 to-cyan-400 text-slate-950 font-black text-xs shadow-lg shadow-cyan-500/20 disabled:opacity-50"
                >
                  Save Expense
                </button>
              </div>
